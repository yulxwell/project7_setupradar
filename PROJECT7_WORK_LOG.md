...
## Finder Advanced Criteria Inline Polish - 2026-06-06 기록

- 상세 기준을 열었을 때 기본 필터가 사라지는 느낌을 줄이기 위해, 기본 필터 카드들은 유지하고 `연결 방식` 카드 자리만 `연결 상세` 카드로 바뀌도록 조정했다.
- Mouse Finder는 손 크기, 형태, 무게 카드가 그대로 보이고, 상세 기준 열림 상태에서 연결 카드만 연결 상세로 전환된다. 배터리/충전, 사이드 버튼, 용도는 아래에 추가 상세 조건으로 붙는다.
...
## Mouse Compare Picker Live QA - 2026-06-07 기록
