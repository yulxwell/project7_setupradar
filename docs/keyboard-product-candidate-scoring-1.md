...
## 10. Product Patch Candidate 1 Manual Apply 결과

2026-06-24 기준 아래 4개를 `review` 상태로 실제 키보드 제품 데이터에 반영했다.
...
- 키보드 제품 수는 12개에서 16개로 증가했다.
- MX Keys S는 기존 MX Keys와 별도 id/slug로 분리했다.
- Apex Pro TKL은 유선 모델 후보로 반영하고 Wireless·Gen 3·구형 모델 차이는 note에 유지했다.
- K3 Max는 K3·K3 Pro와 구분하고 핫스왑·ANSI/ISO·스위치·키캡 옵션 차이를 확인 항목으로 남겼다.
- Air75 V2는 Halo75 V2와 다른 로우프로파일 제품군으로 분리했다.
- HHKB Professional HYBRID Type-S는 `hold`를 유지한다.
- snapshot export와 validator 재실행은 통과했으며 제품 이미지와 링크는 추가하지 않았다.
