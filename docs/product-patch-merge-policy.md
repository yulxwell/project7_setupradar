...
## 13. 향후 Weekly Intake Automation 후보

현재 주간 제품 추가 루틴은 Gemini LLM이 제품 후보를 조사하고 `product_config_patch`를 생성한 뒤, 사람이 JSON 코드블록을 찾아 `tmp/*.json` 파일로 저장하고 validator를 실행하는 방식이다. 이 흐름은 안전하지만 반복되면 복사 실수나 파일 생성 실수가 생길 수 있다.

향후 자동화 후보는 아래처럼 API 연결 없이 로컬 반자동화로 제한한다.

1. Gemini LLM 결과 전체 마크다운을 `tmp/gemini-weekly-result.md`에 저장한다.
2. `npm run product-patch:intake -- ./tmp/gemini-weekly-result.md` 같은 명령을 실행한다.
3. 스크립트가 마크다운 안의 `json` 코드블록 중 `type: "product_config_patch"`인 블록만 찾는다.
4. JSON.parse 검증 후 `tmp/product-patch-weekly-candidates.json`을 생성한다.
5. 제품 수, id, name, category를 요약 출력한다.
6. `--validate` 옵션이 있으면 기존 `product-patch:validate` 흐름을 이어 실행한다.

후보 스크립트 이름:

- `scripts/extract-product-patch-from-markdown.ts`
- `product-patch:extract`
- `product-patch:intake`

단, 이 문서화 시점에는 위 스크립트와 package script를 만들지 않는다. 아직 주 1회 3~5개 제품 수준에서는 수동 복사와 validator 실행으로 충분하며, product patch 형식과 validator 출력이 2~3회 이상 안정적으로 반복된 뒤 구현을 검토한다.

이 intake 후보는 OpenRouter, Gemini, Hermes 같은 외부 API 자동 호출이 아니다. 사람이 Gemini 결과를 받아 로컬 파일에 붙여넣고, 로컬 스크립트가 JSON 추출과 validator 실행만 돕는 중간 단계다.
